...
assert.match(html,/connect-src 'none'/);assert.match(html,/img-src 'self' blob:/);assert.doesNotMatch(html,/img-src 'self' blob: data:/);assert.match(html,/object-src 'none'/);assert.match(html,/frame-src 'none'/);assert.match(html,/form-action 'none'/);assert.match(html,/script-src-attr 'none'/);assert.match(html,/style-src-attr 'none'/);assert.match(html,/Permissions-Policy/);assert.match(html,/href="ai\.txt"/);assert.match(readme,/MAINTENANCE\.md/);assert.match(maintenance,/git revert|Revert/i);assert.match(styles,/min-width:\s*280px/);assert.match(responsive,/max-width:\s*620px/);assert.match(robots,/User-agent: \*/);assert.match(robots,/Sitemap:/);assert.match(ai,/MIT License/);assert.match(ai,/AI visits|AIの訪問/);assert.match(sitemap,/https:\/\/abcderp2\.github\.io\/roof-stretch-editor\//);assert.doesNotMatch(html,/https?:\/\//);
